...
export function uploadCurriculum(formData) {
  return http('/admin/curriculum/upload', {
    method: 'POST',
    body: formData,
  });
}
...
export function updateCurriculum(code, formData) {
  return http(`/admin/curriculum/${encodeURIComponent(code)}`, {
    method: 'PATCH',
    body: formData,
  });
}
...
export function archiveCurriculum(code) {
  return http(`/admin/curriculum/${encodeURIComponent(code)}/archive`, {
    method: 'POST',
  });
}
...
export function cloneCurriculum(body) {
  return http('/admin/curriculum/clone', {
    method: 'POST',
    body,
  });
}
...
export function listCurricula(params = {}) {
  ...
  return http(`/admin/curriculum${qs ? `?${qs}` : ''}`);
}
