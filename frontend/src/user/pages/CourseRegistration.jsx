...
export default function CourseRegistration() {
  ...
  const totalCredits = useMemo(
    () => cart.reduce((sum, c) => sum + (c.credits || 0), 0),
    [cart]
  );
  ...
  const addToCart = (c) => {
    if (
      cart.find(
        (x) => x.courseCode === c.courseCode && x.sectionId === c.sectionId
      )
    )
      return;
    setCart([...cart, c]);
  ...
  const onSubmit = async () => {
    ...
    try {
      const payload = {
        term: filters.term,
        items: cart.map(({ courseCode, sectionId }) => ({
          courseCode,
          sectionId,
        })),
      };
      ...
      setMessage(
        `Đăng ký thành công ${res?.registered || cart.length} học phần.`
      );
      setCart([]);
    } catch (e) {
    ...
    <div style={{ padding: 20 }}>
      ...
      >
        ...
        <div>
          ...
          >
            {(cart || []).length === 0 && (
              <div style={{ color: "#64748b" }}>Chưa có học phần.</div>
            )}
            {(cart || []).map((c, idx) => (
              <div
                key={idx}
            ...
            {message && (
              <div
                style={{
                  marginTop: 8,
                  color:
                    message.includes("thất bại") || message.includes("xung đột")
                      ? "#dc2626"
                      : "#16a34a",
                }}
              >
