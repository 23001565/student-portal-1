...
async function createCourse({ code, name, credits }) {
  ...
  const created = await prisma.course.create({
    ...
    select: { id: true, code: true, name: true, credits: true, archivedAt: true, createdAt: true },
  });
  return created;
}

async function updateCourseByCode(code, { name, credits}) {
  code = cleanStr(code);
  ...
  const updated = await prisma.course.update({ where: { id: course.id }, data, select: { id: true, code: true, name: true, credits: true, archivedAt: true, createdAt: true } });
  return updated;
}
...
async function archiveCourse(code) {
  ...
  const course = await prisma.course.findUnique({ where: { code } });
  if (!course) { const e = new Error('Not found'); e.status = 404; throw e; }
  const updated = await prisma.course.update({ where: { id: course.id }, data: { archivedAt: new Date() }, select: { id: true, code: true, name: true, credits: true, archivedAt: true } });
  return updated;
}
...
async function deleteCourseByCode(code) {
  ...
  // Check if there are any classes for this course (active or historical) or if it's part of any group
  const groups = await prisma.groupCourse.findFirst({ where: { courseId: course.id } });
  ...
  // If no classes, no groups, safe to hard-delete
...
async function uploadCoursesFromCSV(buffer, mapping = {}) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      if (existingId) {
        await prisma.course.update({ where: { id: existingId }, data: { name, credits } });
        updated += 1;
      } else {
        await prisma.course.create({ data: { code, name, credits } });
        inserted += 1;
      }
    } catch (err) {
